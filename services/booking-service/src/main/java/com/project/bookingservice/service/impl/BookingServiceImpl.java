...
public class BookingServiceImpl implements BookingService {
    ...
    public CreateBookingResult createBooking(UUID userId, String idempotencyKey, BookingRequest request) {
        ...
        try {
            ...
            boolean markedFailed = bookingTransactionService.transitionStatus(claimId, BookingStatus.IN_PROGRESS,
        ...
        if (!claimed) {
            ...
            if (current.getStatus() == BookingStatus.CONFIRMED) {
                throw new ConflictException(
                        "Cannot cancel a CONFIRMED booking. Please request a refund instead."
                );
            }
        ...
        if (!completed) {
            ...
            log.error(
                    "CRITICAL: Seats released but booking {} could not transition " +
                            "CANCELLING -> CANCELLED. Manual reconciliation required.", bookingId);
            throw new BookingReconciliationException(
    ...
    @Override
    public void confirmPayment(UUID bookingId, String paymentId) {
        Booking booking = bookingRepository.findById(bookingId)
                .orElseThrow(() -> new RecordNotFoundException("Booking not found: " + bookingId));
        ...
        boolean success = bookingTransactionService.confirmPayment(
                bookingId,
                paymentId,
                LocalDateTime.now()
        );
        ...
        if (!success) {
            throw new ConflictException("Failed to confirm booking due to concurrent update");
        }
    ...
    private CreateBookingResult handleFinalizeFailure(
        ...
        if (persisted != null
            ...
            if (!markedFailed) {
                ...
                log.error("CRITICAL: Could not transition booking {} " + "IN_PROGRESS -> FAILED", claimId);
            }

            log.error("CRITICAL: Booking {} requires manual reconciliation. " + "Seats may remain reserved.", claimId
            );
